...
LANDISDisp <- function(sim, dtSrc, dtRcv, pixelGroupMap, species,
                       ...
                       maxPotentialsLength = 1e3, verbose = FALSE, useParallel, ...) {
  ...
  # seedSourceMap <- rasterizeReduced(
  #   speciesSrcPool,
  #   fullRaster = pixelGroupMap,
  #   mapcode = "pixelGroup",
  #   newRasterCols = "speciesSrcPool"
  # )
  # seedReceiveMap <- rasterizeReduced(
  #   speciesRcvPool,
  #   fullRaster = pixelGroupMap,
  #   mapcode = "pixelGroup",
  #   newRasterCols="speciesRcvPool"
  # )
  seedSourceMaps <- lapply(seedSourceMaps, function(x)
  ...
  xysAll <- xyFromCell(seedSourceMaps$speciesSrcPool,
  ...
  if (is.logical(useParallel) | is.numeric(useParallel)) {
    ...
    stop(
      "Please specify the useParallel argument correctly. Currently, it takes either numeric, logical or cluster object"
    )
  }
...
speciesComm <- function(num, sc) {
  ...
  speciesCode <- lapply(indices, function(x)
...
seedDispInnerFn <- function(activeCell, potentials, n, speciesRcvPool, sc,
  ...
  while (NROW(potentials) &
    ...
    shortPotentials <- setkey(potentials, fromInit, to) %>%
      ...
      .[, list(fromInit, to)]
    set(shortPotentials, , "dis", as.integer(pointDistance(xysAll[shortPotentials[, fromInit], ],
    ...
    if (n - cellSize == 0) {
      ...
      potentials <- shortPotentials[((n - cellSize) %<=% dis) &
                                      (dis %<=% n), ][potentials, nomatch =
                                                        0][(dis %<=% maxDist), ]
    } else {
      potentials <- shortPotentials[((n - cellSize) %<<% dis) &
                                      (dis %<=% n), ][potentials, nomatch =
    ...
    if (NROW(potentials) > 0) {
      ...
      if (NROW(potentialsWithSeedDT) > 0) {
        ...
        potentialsWithSeedDT <-
          potentialsWithSeedDT[, .(receivesSeeds = runif(nr) < dispersalProb,
                                   fromInit,
                                   speciesCode)]
