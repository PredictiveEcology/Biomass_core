test_that("test Biomass_coreInit", {
  ...
  initialCommunities <- data.table(
    mapcode = c(1, 2, 2), description = "NA",
    species = c("pinubank", "pinubank", "abiebals"),
    age1 = c(3, 7, 10), age2 = c(4, 12, 19)
  )
  ecoregion <- data.table(
    active = c("yes", "yes", "no"), mapcode = 1:3,
    ecoregion = c("eco1", "eco2", "eco3"), AET = 600, description = "NA"
  )
  species <- data.table(
    species = c("abiebals", "pinubank"), longevity = c(200L, 100L),
  ...
  speciesEcoregion <- data.frame(expand.grid(
    year = 0, ecoregion = c("eco1", "eco2"),
  ...
  cohortDataShort_compared <- data.table(
    pixelGroup = c(11, 12, 12, 12, 12, 21, 22, 22, 22, 22),
    speciesCode = c(
      2, 1, 1,
      2, 2, 2,
      1, 1, 2, 2
    ...
    B = c(
      5028, 1065, 8523, 1559, 3454, 4515, 960,
      7718, 1399, 3079
    )
  )
  ...
  expect_equal(
    getValues(simOutput$pixelGroupMap),
  ...
  expect_equal(
    names(simOutput$spinupOutput),
    c(
      "pixelGroup", "species", "age",
  ...
  expect_equal(
    names(simOutput$simulationTreeOutput),
    c(
      "Year", "siteBiomass", "Species", "Age", "iniBiomass",
  ...
  expect_equal(
    names(simOutput$regenerationOutput),
  ...
  speciesEcoregion_revised <- simOutput$speciesEcoregion[, .(
    year, ecoregion, species,
    establishprob, maxANPP, maxB
  )]
  speciesEcoregion_compared <- data.table(
    year = 0,
