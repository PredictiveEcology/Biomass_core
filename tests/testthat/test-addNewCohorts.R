test_that("test add new cohort function",{
  ...
  newcohortdata1 <- rbindlist(list(cohortData,cohortData,cohortData))[,':='(speciesCode=c(7,1,7),
                                                                 pixelIndex=c(7:8,8))]
  newcohortdata2 <- rbindlist(list(cohortData,cohortData,
                                   cohortData,cohortData))[
                                     ,':='(pixelGroup=2,
                                           ...
                                           pixelIndex=c(4:6,6))]
  newcohortdata <- rbindlist(list(newcohortdata1,newcohortdata2))[,':='(age = NULL, B = NULL,
                                                                        sumB = NULL, aNPPAct = NULL,
                                                                        mortality = NULL)]
