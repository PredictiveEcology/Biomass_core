...
test_that("calculateCompetition() matches the reference bAP and bPM at both stages", {
  ...
  ref <- function(bAP, bPM) {
    setkey(data.table(pixelGroup = c(rep(1, 13), rep(2, 10)),
                      age = c(rep(0, 5), 6:10, 20, 30, 50, rep(0, 4), 5:10), bAP = bAP, bPM = bPM),
           pixelGroup, age)
  }
  got <- function(output) {
    out <- setkey(output[, .(pixelGroup, age, bAP, bPM)], pixelGroup, age)
    out[, `:=`(bAP = round(bAP, 4), bPM = round(bPM, 4))]
    out
  }
  expect_equal(got(calculateCompetition(cohortData, stage = "spinup")),
               ref(bAP = c(rep(NA, 5), 0.6, 0.6364, 0.6667, 0.6923, 0.7143, 0.7333,
  ...
  expect_equal(got(calculateCompetition(cohortData, stage = "nonSpinup")),
               ref(bAP = c(0.2, 0.3333, 0.4286, 0.5, 0.5556, 0.6, 0.6364, 0.6667, 0.6923,
