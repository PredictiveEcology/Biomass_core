...
author: "`r paste(unname(vapply(as.character(SpaDES.core::moduleMetadata(module = 'Biomass_core', path = '..')$authors), function(x) { strsplit(x, split = ' <')[[1]][1] }, character(1))), sep = ', ')`"
...
```{r setupBiomassCore, include=FALSE}
...
```{r figBiomassCore, echo = FALSE, eval = TRUE, out.width = "60%", fig.align = 'center', fig.cap = "_Biomass_core_ simulates tree cohort growth, mortality, recruitment and dispersal dynamics, as a function of  cohort ageing and competition for light (shading) and space, as well as disturbances like fire (simulated using other modules)."}
...
```{r moduleInputsBiomassCore, echo = FALSE, eval = TRUE, message = FALSE}
df_inputs <- SpaDES.core::moduleInputs("Biomass_core", "..")[, c(1, 3)]  ## name + desc only
knitr::kable(df_inputs,
             caption = "List of _Biomass_core_  input objects and their description.") %>%
...
```{r moduleParamsBiomassCore, echo = FALSE, eval = TRUE, message = FALSE}
df_params <- SpaDES.core::moduleParams("Biomass_core", "..")[, c(1, 6)] ## name + desc only
knitr::kable(df_params,
             caption = "List of _Biomass_core_ parameters and their description.") %>%
  kableExtra::kable_styling(latex_options = "scale_down", full_width = TRUE)
```
...
```{r moduleOutputsBiomassCore, echo = FALSE, eval = TRUE, message = FALSE}
df_outputs <- SpaDES.core::moduleOutputs("Biomass_core", "..")[, c(1, 3)] ## name + desc only
knitr::kable(df_outputs,
             caption = "List of _Biomass_core_ output objects and their description.") %>%
  kableExtra::kable_styling(latex_options = "scale_down", full_width = TRUE)
```
...
```{r moduleInputs2BiomassCore, echo = FALSE, eval = TRUE, message = FALSE}
...
```{r moduleParams2BiomassCore, echo = FALSE, eval = TRUE, message = FALSE}
...
```{r load-SpaDESBiomassCore, eval=FALSE}
library(SpaDES)
...
```{r module usage example setup BiomassCore, eval=FALSE}
tempDir <- tempdir()
...
```{r module usage example run BiomassCore, eval=FALSE}
...
```{r figBiomassCoreOutPlots, echo = FALSE, eval = TRUE, fig.show='hold', fig.cap = "_Biomass_core_ automatically generates simulation visuals of species dynamics across the landscape in terms of total biomass, number of presences and age and productivity (above), as well as yearly plots of total biomass, productivity, mortality, reproduction and leading species in each pixel (below)."}
...
```{r figLBSEtest3, echo = FALSE, eval = TRUE, out.width = "60%", fig.align = 'center', fig.cap = "Hashing design for _Biomass_core_. In the re-coded _Biomass_core_, the pixel group map was hashed based on the unique combination of species composition (i.e., community map) and ecolocation map, and associated with a lookup table. The subfigure in the right upper corner was the original design that linked the map to the lookup table by pixel key."}
