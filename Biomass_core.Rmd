...
The biggest differences between LBMR and LANDIS-II lie on how the model is parametrised and initialized and how different processes have been compartmentalised in order for higher flexibility.
...
## April 1st, 2019:
##  growth/mortality processes still in separate module, but will be ported back to LBMR soon.
moduleName <- c("LBMR", "LandR_BiomassGMOrig")  
spadesModulesDirectory <- "~/LBMR_test/modules" # where the module will be downloaded
```
...
setPaths(inputPath = "~/LBMR_test/inputs", 
         cachePath = "~/LBMR_test/cache", 
         modulePath = spadesModulesDirectory, 
         outputPath = "~/LBMR_test/outputs")
