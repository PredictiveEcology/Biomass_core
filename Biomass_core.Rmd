...
By default, *Biomass_core* obtains trait values from available LANDIS-II tables (see Table \@ref(tab:moduleInputs2-Biomass-core)), but traits can be adjusted/added directly by the user or by other modules. For instance, using the *Biomass_borealDataPrep* will adjust some trait values for Western Canadian boreal forests (e.g., longevity values are adjusted following Burton and Cumming <!-- REF -->, while using *Biomass_speciesParameters* will calibrates the `growthcurve` and `mortalityshape` parameters and adds two additional species traits (`inflactionFactor` and `mANPPproportion`) that influence `maxB` and `maxANPP` (respectively).
